...
又例如 $\textit{nums}=[a,b,c,d,e]$，最终得到 $[a+4b+6c+4d+e]$。

观察系数，看上去是杨辉三角（组合数），为什么？

![lc2221.png](https://pic.leetcode.cn/1757988106-DlcZTQ-lc2221.png){:width=260}

比如第一排的 $\textit{nums}[1]=2$，在最终答案中的系数是 $4$，这是怎么算的？

$2$ 可以往左贡献给下一排的 $3$，也可以往右贡献给下一排的 $5$，相当于分裂了一次，多了一个 $2$；下一排的 $5$ 也会分裂一次，这就又多了一个 $2$（这个 $2$ 在 $5$ 里面）。下下一排的 $2$ 也会分裂一次，这就又多了一个 $2$。所以一共有 $4$ 个 $2$。

更形象的理解方法是，想象我们从第一排的 $2$ 移动到底部：

- 往左下走，就是原数。
- 往右下走，就是分裂出一个新的 $2$。

所以 $2$ 在最终答案中的系数，等于从第一排的下标 $i=1$ 移动到底部的**路径数**。

一般地，由于移动范围是一个平行四边形，根据 [62. 不同路径题解](https://leetcode.cn/problems/unique-paths/solutions/3062432/liang-chong-fang-fa-dong-tai-gui-hua-zu-o5k32/) 的方法二，我们要走 $n-1$ 步，其中恰好有 $i$ 步要往左下走，恰好有 $n-1-i$ 步要往右下走。所以在最终答案中，$\textit{nums}[i]$ 的系数是组合数
...
把每个数中的因子 $2$ 和 $5$ 提取出来，单独统计因子个数。一个数去掉其中所有因子 $2$ 和 $5$ 之后，得到的整数 $a$ 与 $10$ 互质，这样就可以用**欧拉定理**计算整数 $a$ 在模 $10$ 下的逆元，即 $a^{\varphi(10)-1} = a^3$。

### 细节

预处理 $2$ 的幂模 $10$：由于 $2^i\bmod 10\ (i>0)$ 按照 $2,4,8,6$ 的周期循环，所以只需预处理一个长为 $4$ 的数组。
