...
- 如果 $x=0$ 且 $k$ 是奇数，或者 $x=1$ 且 $k$ 是偶数，那么前面的操作执行完后 $\textit{nums}[i]$ 变成了 $1$。所以如果 $x\ne k\bmod 2$，则不需要操作。
- 如果 $x=0$ 且 $k$ 是偶数，或者 $x=1$ 且 $k$ 是奇数，那么前面的操作执行完后 $\textit{nums}[i]$ 变成了 $0$。所以如果 $x= k\bmod 2$，则一定要操作。
