...
- 填 $2$，那么前面不会有比 $2$ 大的数，这意味着剩下的 $n-1=2$ 个数的逆序对需要等于 $2-0=2$。（这是不存在的）
- 填 $1$，那么前面一定有 $1$ 个比 $1$ 大的数，这意味着剩下的 $n-1=2$ 个数的逆序对需要等于 $2-1=1$。（这只有 $1$ 种构造方式，即 $[2,0]$）
- 填 $0$，那么前面一定有 $2$ 个比 $0$ 大的数，这意味着剩下的 $n-1=2$ 个数的逆序对需要等于 $2-2=0$。（这只有 $1$ 种构造方式，即 $[1,2]$）
...
考虑第 $i$ 个数 $\textit{perm}[i]$ 和前面 $\textit{perm}[0]$ 到 $\textit{perm}[i-1]$ 可以组成的逆序对的个数：
