...
**定理**：有序子数组的 LCP，等于子数组第一个字符串和最后一个字符串的 LCP。
...
- 如果删除的字符串不在 $[\textit{mxI},\textit{mxI}+k-1]$ 中，答案是 $\textit{mx}$。
- 如果删除的字符串在 $[\textit{mxI},\textit{mxI}+k-1]$ 中，答案是 $\textit{mx}_2$。

**证明**：

首先，不可能因为删除了一个字符串，导致 LCP 变得更长。删除字符串并不是由 $k$ 个变成 $k-1$ 个，而是还需要找一个在子数组外面的字符串，恢复成 $k$ 个字符串。（反证法）假如恢复成 $k$ 个后，LCP 能变长，那么为了让 LCP 尽量长，应当选与子数组相邻的字符串，即下标为 $\textit{mxI}-1$ 或者 $\textit{mxI}+k$ 的字符串。如果恢复后得到了更长的 LCP，长度为 $m$，那么在删除前我们也能得到长为 $m$ 的 LCP，也就是选择以 $\textit{mxI}-1$ 开头的或者以 $\textit{mxI}+k$ 结尾的长为 $k$ 的子数组，可以得到长为 $m$ 的 LCP，矛盾。

其次，不可能因为删除了一个字符串，导致 LCP 长度比 $\textit{mx}_2$ 还大。（反证法）如果出现这种情况，说明以 $\textit{mxI}-1$ 开头的或者以 $\textit{mxI}+k$ 结尾的长为 $k$ 的子数组，LCP 长度比 $\textit{mx}_2$ 还大，那我们先前算出来的 $\textit{mx}_2$ 也应当来自这些子数组，不可能来自其他 LCP 更小的子数组，矛盾。

下面说明答案可以等于 $\textit{mx}_2$。换句话说，删除一个字符串不会导致 $\textit{mx}_2$ 变小。

- 如果次大子数组不包含删除的字符串，那么答案是 $\textit{mx}_2$。
- 如果次大子数组包含删除的字符串（记作 $s$），说明最大子数组和次大子数组都包含 $s$，这两个子数组是相交的（有公共部分）。在这种情况下，**次大子数组的 LCP，是最大子数组的 LCP 的前缀**。所以从次大子数组中删除 $s$，再增加一个在最大子数组中的字符串（这样仍然有 $k$ 个字符串），相当于引入了一个前缀包含次大子数组 LCP 的字符串，所以次大子数组的 LCP 是保持不变的，$\textit{mx}_2$ 不会变小，所以答案是 $\textit{mx}_2$。
