...
- 如果 Alice 把左半中的一个问号改成 $x$，那么 Bob 可以：
    - 把右半中的一个问号改成 $x$，那么 $D' = D + x - x + 0 = D = 0$。注：由于左半右半各少了一个问号，所以 $\dfrac{q'_L - q'_R}{2}\cdot 9 = 0$。
    - 或者，把左半中的一个问号改成 $9-x$，那么 $D' = D + x + (9-x) - 9 = D = 0$。注：由于左半少了两个问号，所以 $\dfrac{q'_L - q'_R}{2}\cdot 9 = -9$。
...
定义 $X_i$ 为左半第 $i$ 个字符的权值，$Y_i$ 为右半第 $i$ 个字符的权值。字符权值的定义见写法二。
...
定义 $Z_i = X_i - Y_i$。由于 $X_i$ 与 $Y_i$ 同分布且独立，所以有
...
设 $m = n/2$，$S = \sum\limits_{i=1}^m Z_i$。我们有
...
如何用 $f(x)$ 计算 $P(S=0)$？$f(x)$ 是连续的，但 $S$ 是个离散的整数。处理方法是把 $S=0$ 视作 $-0.5\le S\le 0.5$，即
