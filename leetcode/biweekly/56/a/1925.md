...
更仔细地考察 $\left\lfloor\dfrac{2n}{u^2+v^2}\right\rfloor$：

- 当 $u^2+v^2\le \sqrt[3] {2n}$ 时，暴力枚举 $\sqrt[3] {2n}$ 以内的 $u$ 和 $v$，只需要枚举 $\mathcal{O}(n^{2/3})$ 次。
- 当 $u^2+v^2 > \sqrt[3] {2n}$ 时，$\dfrac{2n}{u^2+v^2} < (2n)^{2/3}$，所以 $\left\lfloor\dfrac{2n}{u^2+v^2}\right\rfloor$ 只有 $\mathcal{O}(n^{2/3})$ 个不同的整数。横看成岭侧成峰，考虑对这 $\mathcal{O}(n^{2/3})$ 个不同的整数，计算出有多少个 $(u,v)$。

推荐读者先看 [数论分块](https://oi-wiki.org/math/number-theory/sqrt-decomposition/)，先理解怎么算 $\displaystyle\sum_{i=1}^{n} \left\lfloor\dfrac{n}{i}\right\rfloor$，再理解更复杂的 $\left\lfloor\dfrac{2n}{u^2+v^2}\right\rfloor$。
...
$[1,R]$ 内的与 $u$ 互质的奇数个数，等于 $[1,R]$ 内的与 $u$ 互质的整数个数，减去 $[1,R]$ 内的与 $u$ 互质的偶数个数。对于后者，设偶数为 $2k$，由于 $u$ 是奇数，所以 $\gcd(2k,x) = \gcd(k,x)$。由于 $2k\le R$，所以 $k\le \lfloor\frac{R}{2}\rfloor$。所以 $[1,R]$ 内的与 $u$ 互质的偶数个数等于 $[1,\lfloor\frac{R}{2}\rfloor]$ 内的与 $u$ 互质的整数个数。
...
预处理的时空复杂度为 $\mathcal{O}(\sqrt N\log N)$，不计入。

- 时间复杂度：$\mathcal{O}(n^{2/3} \log n)$。当 $u\le n^{1/3}$ 时，枚举 $u$ 和 $v$ 是 $\mathcal{O}(n^{2/3})$ 的。当 $u > n^{1/3}$ 时，$\left\lfloor\dfrac{n}{u^2+v^2}\right\rfloor$ 有 $\mathcal{O}\left(\dfrac{n}{u^2}\right)$ 种不同的值。用积分估计，$\displaystyle \int_{n^{1/3}}^{n^{1/2}} \dfrac{n}{u^2} \, \mathrm{d}u = n^{2/3} - n^{1/2}$，所以当 $u > n^{1/3}$ 时的循环次数为 $\mathcal{O}(n^{2/3})$。由于 $\sqrt {2n}$ 以内正整数的平均因子个数是 $\mathcal{O}(\log n)$ 的（即使只考虑 $\mu(d)\ne 0$ 的因子，平均因子个数仍然是 $\mathcal{O}(\log n)$ 的），所以总的时间复杂度为 $\mathcal{O}(n^{2/3} \log n)$。
- 空间复杂度：$\mathcal{O}(1)$。
