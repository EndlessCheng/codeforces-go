...
> 注：下面讨论的「下标」是指排序后的数组元素下标。

在第 $t$ 秒，$s_1 + s_2\cdot t$ 减少量的最大值相当于求解如下问题：

从 $\textit{nums}_1$ 中选择一个长为 $t$ 的子序列，作为要操作的数。其中，子序列第 $j$ 个数的 $\textit{nums}_2[i]$ 的系数为 $j$。计算减少量的最大值。

> 注：排序目的是确保子序列第 $j$ 个数的系数「恰好」是 $j$。为什么？请看排序不等式。
...
类似 [0-1 背包](https://www.bilibili.com/video/BV16Y411v7Y6/)，定义 $f[i+1][j]$ 表示从下标为 $0,1,2,\cdots,i$ 的数中选出 $j$ 个数（$j\le i+1$），减少量最大是多少。

考虑下标为 $i$ 的数「选或不选」：

- 不选，问题变成从下标为 $0,1,2,\cdots,i-1$ 的数中选出 $j$ 个数，减少量最大是多少，即 $f[i+1][j] = f[i][j]$。
- 选，问题变成从下标为 $0,1,2,\cdots,i-1$ 的数中选出 $j-1$ 个数，减少量最大是多少，即 $f[i+1][j] = f[i][j-1] + \textit{nums}_1[i] + \textit{nums}_2[i] \cdot j$。
- 这两种情况取最大值，即
...
初始值 $f[0][0]=0$。没有数字，选 $0$ 个数，减少量是 $0$。
