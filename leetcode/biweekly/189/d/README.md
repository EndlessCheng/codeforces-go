...
假设有三个请求，完成的时间分别为 $2,4,5$，总惩罚为 $2+4+5=11$。如果在计算过程中维护当前时间，状态就太多了。

横看成岭侧成峰，考虑每过一秒，总惩罚增加了多少。上面的例子一共用了 $5$ 秒，每一秒总惩罚的增量分别为 $3,3,2,2,1$，总和为 $3+3+2+2+1=11$。如此计算，我们只需考虑**还剩下多少个未完成的请求**，即为每过一秒总惩罚的增量。

## 二、寻找子问题

暴力的想法是，枚举 $\textit{requests}$ 的全排列，但这显然太慢了。

设 $a,b,c$ 是 $\textit{requests}$ 中的三个数，且 $a < b < c$。在从楼层 $a$ 移动到楼层 $c$ 的过程中，一定会经过楼层 $b$，我们楼层 $b$ 的请求可以顺带处理。这意味着，我们可以把 $a\to c$ 的过程拆分成 $a\to b\to c$。
...
- 把电梯移到 $\textit{requests}[2]$，问题变成处理完了 $\textit{requests}$ 的连续子数组 $[2,6]$，且电梯位于左端点，处理剩余请求的最小总惩罚。这里的总惩罚指的是惩罚增量之和，下同。
