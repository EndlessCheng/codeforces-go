...
由于两个集合的大小都是 $n$，根据 $\textit{diff}$ 的定义，$\textit{diff}$ 的总和（出现次数之差的总和）是 $n-n=0$。换句话说，$\textit{diff}$ 中的正数之和等于负数之和。**集合 $A$ 多出的数，恰好也是集合 $B$ 多出的数**。把这些数交换，即可让 $A=B$。由于交换一次，可以让 $\textit{diff}[x]$ 中的正数之和减少 $1-(-1) = 2$，所以交换次数等于 $\textit{diff}$ 中的正数之和除以 $2$。在上面的例子中，$\textit{diff}[x]=6$，需要交换 $3$ 次。
