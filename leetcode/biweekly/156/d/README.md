...
- 子树 $x$ 的点权和 $s$。
- $x$ 的祖先节点反转了偶数次时，在子树 $x$ 中可以得到的最大增量 $\textit{res}_0$。
- $x$ 的祖先节点反转了奇数次时，在子树 $x$ 中可以得到的最大增量 $\textit{res}_1$。

最终答案为根节点的 $s+\textit{res}_0$。根节点没有祖先节点，相当于祖先节点的反转次数一定是偶数。

设 $x$ 的儿子为 $y$。

**不反转** $x$，问题变成儿子 $y$ 的最大增量：

- 对于 $y$ 来说，祖先节点反转次数的奇偶性不变。
...
- 如果 $x$ 的祖先节点反转了偶数次，反转 $x$ 会立刻得到 $-2s$ 的增量。
- 如果 $x$ 的祖先节点反转了奇数次，反转 $x$ 会立刻得到 $2s$ 的增量。
...
- 如果 $x$ 的祖先节点反转了偶数次，反转 $x$ 后，对于 $x$ 的 $k$ 级后代来说，祖先节点反转了奇数次，所以累加的是 $x$ 的所有 $k$ 级后代的 $\textit{res}_1$。
- 如果 $x$ 的祖先节点反转了奇数次，反转 $x$ 后，对于 $x$ 的 $k$ 级后代来说，祖先节点反转了偶数次，所以累加的是 $x$ 的所有 $k$ 级后代的 $\textit{res}_0$。

第一部分与第二部分相加（按照 $x$ 的祖先节点反转次数的奇偶性分开计算），结果分别记作 $\textit{inv}_0$ 和 $\textit{inv}_1$。
...
最后，如何累加 $x$ 的所有 $k$ 级后代 $z$ 的 $\textit{res}_i$？站在 $x$ 的视角计算，很不方便（$z$ 的分布是分散的）；但如果站在 $z$ 的视角，去更新 $x$，就很好算了（每个 $z$ 至多有一个 $k$ 级祖先 $x$）。
...
class Solution {
    ...
    private long[] dfs(int x, int fa, List<Integer>[] g, int[] nums, int k, List<long[]> f) {
        ...
        long s = nums[x]; // 子树和
        long notInv0 = 0; // 不反转 x 时的额外增量（0 表示上面反转了偶数次，1 表示上面反转了奇数次）
        long notInv1 = 0; 
        for (int y : g[x]) {
            if (y == fa) {
                continue;
