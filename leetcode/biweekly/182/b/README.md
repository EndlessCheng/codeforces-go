...
如果 $s$ 中 $\texttt{0}$ 和 $\texttt{1}$ 都有，那么 $\texttt{0}$ 在左 $\texttt{1}$ 在右，意味着 $s$ 包含子序列 $\texttt{01}$；$\texttt{1}$ 在左 $\texttt{0}$ 在右，意味着 $s$ 包含子序列 $\texttt{10}$。所以修改后的 $s$ 要么全为 $\texttt{0}$，要么全为 $\texttt{1}$。

**思考这个更简单的问题，可以启发我们往全为 $\texttt{0}$ 或者全为 $\texttt{1}$ 的方向思考**。

回到本题。不能包含子序列 $\texttt{110}$ 或 $\texttt{011}$，意味着对于修改后的 $s$ 中的任意一个 $\texttt{0}$，其左侧不能有超过一个 $\texttt{1}$，右侧也不能有超过一个 $\texttt{1}$。这样的 $s$ 长什么样？
