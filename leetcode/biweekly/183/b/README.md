...
**核心思路**：如果 $x$ 是集中位置，那么在 $x+k/2$ 处断开环。把 $x$ 右手边的到 $x$ 距离在 $k/2$ 以内的数，从右边移动到 $x$；其余数从左边移动到 $x$。

**注**：这意味着，在 $x$ 对面的那段弧是没人经过的，那么断开这段弧，变成一个普通数组问题。根据 [中位数贪心](https://zhuanlan.zhihu.com/p/1922938031687595039)，可以都变成 $a$ 中的某个元素。

枚举 $a[i]$ 作为集中的位置，那么在 $[a[i], a[i]+k/2]$ 中的数，**减少**至 $a[i]$ 更好（相比增加）；在 $[a[i]-k/2, a[i]]$ 中的数，**增加**至 $a[i]$ 更好。考虑到 $a[i]-k/2$ 可能是负数，为方便计算，我们可以改成等价写法：把 $[a[i]+k/2, a[i]+k]$ 中的数增大至 $a[i]+k$。**注**：在模 $k$ 意义下，$x$ 和 $x+k$ 是同一个位置。
