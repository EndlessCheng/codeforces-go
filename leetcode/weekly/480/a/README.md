...
public:
    int absDifference(vector<int>& nums, int k) {
        ranges::sort(nums);
        return reduce(nums.end() - k, nums.end()) - reduce(nums.begin(), nums.begin() + k);
    }
};
```
...
public:
    int absDifference(vector<int>& nums, int k) {
        ...
        int ans = reduce(nums.end() - k, nums.end());
        ranges::nth_element(nums, nums.begin() + k);
        return ans - reduce(nums.begin(), nums.begin() + k);
    }
};
```
...
- 时间复杂度：$\mathcal{O}(n\log n)$ 或 $\mathcal{O}(n)$，其中 $n$ 是 $\textit{nums}$ 的长度。使用快速选择算法可以做到 $\mathcal{O}(n)$，见 C++ 第二份代码。
