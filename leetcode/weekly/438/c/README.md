...
如果终点是最底部，那么 $2$ 在最后操作得到的数中的系数，等于从第一排的下标 $i=1$ 移动到最底部的**路径数**。

设 $m$ 是数组长度。一般地，由于移动范围是一个平行四边形，根据 [62. 不同路径题解](https://leetcode.cn/problems/unique-paths/solutions/3062432/liang-chong-fang-fa-dong-tai-gui-hua-zu-o5k32/) 的方法二，我们要走 $m-1$ 步，其中恰好有 $i$ 步要往左下走，恰好有 $m-1-i$ 步要往右下走。所以在最后操作得到的数中，$a[i]$ 的系数是组合数
