...
这意味着，如果第一个数或者最后一个数已经在正确的位置上，那么至多操作 $1$ 次。否则至少操作 $2$ 次（$n\ge 3$）。

分类讨论（从上到下依次判断）：

- **情况一**：如果 $s$ 已经是升序，无需操作。
- **情况二**：如果 $n=2$，无法排序。
- **情况三**：第一个数或者最后一个数已经在正确的位置上。
    - 如果 $s[0]$ 是最小值，排序 $[1,n-1]$ 即可，操作 $1$ 次。
    - 如果 $s[n-1]$ 是最大值，排序 $[0,n-2]$ 即可，操作 $1$ 次。
- **情况四**：只需 $1$ 次操作，就可以变成情况三。
    - 如果 $[1,n-2]$ 中有最小值，那么先排序 $[0,n-2]$，把最小值排到最前面，变成情况三，排序 $[1,n-1]$ 即可，操作 $2$ 次。
    - 如果 $[1,n-2]$ 中有最大值，那么先排序 $[1,n-1]$，把最大值排到最后面，变成情况三，排序 $[0,n-2]$ 即可，操作 $2$ 次。
