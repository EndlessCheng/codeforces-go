...
这意味着，在 $c_1$ 很大 $c_2$ 很小的情况下（比如 $c_1=999,\ c_2=1$），全部变成一个比 $M$ 更大的数，有机会得到一个更小的总开销。

那么，都变成哪个数，可以得到最小的总开销呢？

分类讨论。

## 情况一

当 $n\le 2$ 时，只能用操作一，直接返回 $(M-m)\cdot c_1$。

下面的讨论 $n\ge 3$。

## 情况二

如果 $2c_1\le c_2$，那么用操作二不如用两次操作一，直接把所有数一个一个地增加，都变成 $M$，总共需要执行
...
如果 $2c_1> c_2$，操作二更划算，那么**应当尽量多地使用操作二**。

设所有数都变成 $x\ (x\ge M)$，那么所有数都需要在 $M$ 的基础上额外增加 $x-M$，如果只执行操作一，就需要执行
...
又例如现在有三个盒子，分别装有 $4,1,1$ 个小球，我们可以：

- 从第 $1,2$ 个盒子中各取一个小球，剩余个数为 $3,0,1$。
- 从第 $1,3$ 个盒子中各取一个小球，剩余个数为 $2,0,0$。
- 无法继续用操作二，只能用操作一。

看上去，如果一个盒子装有很多小球，我们无法只用操作二完成。

总共有 $s$ 个小球，一个盒子最多有 $d = x - m$ 个小球。

**定理**：如果 $d\le s- d$，那么可以只用操作二，最后至多剩下一个小球。否则，在执行 $s-d$ 次操作二后，剩余的 $d-(s-d)$ 个小球只能用操作一完成。

**证明**：构造。以 $3,3,2$ 为例，总共有 $3+3+2=8$ 个位置，我们可以按照 $1,3,5,7,2,4,6,8$ 的顺序，首先填入三个 $1$，得到
...
如果 $d> s- d$，即使每次都用操作二从这个装有 $d$ 个小球的盒子中取球，在 $s-d$ 次操作二后，其余盒子中的总共 $s-d$ 个小球全部取完，后面只能用操作一取走这个盒子中剩余的 $d-(s-d)$ 个小球。

综上所述，如果 $2c_1> c_2$：

- 如果 $2d\le s$，那么先执行 $\left\lfloor\dfrac{s}{2}\right\rfloor$ 次操作二，然后执行 $s\bmod 2$ 次操作一，总开销为
   $$
   \left\lfloor\dfrac{s}{2}\right\rfloor\cdot c_2 + s\bmod 2\cdot c_1 
   $$
- 如果 $2d> s$，那么先执行 $s-d$ 次操作二，然后执行 $2d-s$ 次操作一，总开销为
  $$
  (s-d)\cdot c_2 + (2d-s)\cdot c_1
  $$
...
> 注意当 $n\ge 3$ 时，上式 $\le 2M$，所以也可以枚举到 $2M$。
...
如果函数交点横坐标 $\le M$，那么 $x$ 只需枚举 $M$ 和 $M+1$；否则 $x$ 应当枚举交点横坐标附近的数（右图），以及 $M$（左图）。

设 $i = \left\lceil\dfrac{nM-2m-\textit{base}}{n-2}\right\rceil = \left\lfloor\dfrac{nM-2m-\textit{base}+n-3}{n-2}\right\rfloor$，那么答案为
...
public:
    int minCostToEqualizeArray(vector<int>& nums, int c1, int c2) {
        ...
        long long base = n * M - accumulate(nums.begin(), nums.end(), 0LL);
        if (n <= 2 || c1 * 2 <= c2) {
            return base * c1 % MOD;
        }
