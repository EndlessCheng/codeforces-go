...
事实上，倒推的过程中不会出现 $\textit{sz}=0$ 的情况。如果出现，说明我们把前面的字母全部删掉了，答案不存在。但代码提前判断了 $k$ 下标越界的情况，如果 $k$ 没有越界，则答案一定存在，矛盾，所以倒推的过程中 $\textit{sz}$ 始终为正（在每次循环的开头）。`sz = max(sz - 1, 0)` 可以直接改成 `sz -= 1`。
