直接从左到右找回文串，需要知道后续路径的完整顺序信息（因为要判断是否回文），考虑完全图，这有 $\mathcal{O}(n!)$ 个，太多了。

**核心思路**：回文串问题，可以**枚举回文中心**，从中心向左右两边扩展（中心扩展法）。

已知我们访问了节点集合 $S$，那么 $S$ 中的节点的访问顺序是不重要的，我们只需要知道访问过哪些点。这样就无需暴力枚举节点访问顺序的排列了，只需维护节点无序集合的信息，可以用状压 DP 解决。

> 什么情况下会产生重叠子问题，可以看 [视频讲解](https://www.bilibili.com/video/BV1xSuFzHEa1/?t=28m51s) 中画的图。
