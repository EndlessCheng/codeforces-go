...
因此，定义状态为 $\textit{dfs}(i,j,k)$，表示在 $[0,i]$ 中选数字，且在 $[i+1,n-1]$ 中选的两个不重叠子序列的 GCD 分别为 $j,k$ 时，最终得到的合法子序列对的个数。

接下来，思考如何从一个状态转移到另一个状态。

讨论 $x=\textit{nums}[i]$ **选或不选**：

- 不选，不把 $x$ 加到任何子序列中，那么需要解决的问题为：在 $[0,i-1]$ 中选数字，且在 $[i,n-1]$ 中选的两个不重叠子序列的 GCD 分别为 $j,k$ 时，最终可以得到的合法子序列对的个数，即 $\textit{dfs}(i-1,j,k)$。
- 选，把 $x$ 加到第一个子序列中，那么需要解决的问题为：在 $[0,i-1]$ 中选数字，且在 $[i,n-1]$ 中选的两个不重叠子序列的 GCD 分别为 $\gcd(j,x),k$ 时，最终可以得到的合法子序列对的个数，即 $\textit{dfs}(i-1,\gcd(j,x),k)$。
- 选，把 $x$ 加到第二个子序列中，那么需要解决的问题为：在 $[0,i-1]$ 中选数字，且在 $[i,n-1]$ 中选的两个不重叠子序列的 GCD 分别为 $j,\gcd(k,x)$ 时，最终可以得到的合法子序列对的个数，即 $\textit{dfs}(i-1,j,\gcd(k,x))$。

三种选法互斥，根据加法原理，三者相加得

$$
\textit{dfs}(i,j,k) = \textit{dfs}(i-1,j,k) + \textit{dfs}(i-1,\gcd(j,x),k) + \textit{dfs}(i-1,j,\gcd(k,x))
$$
...
具体来说，$f[i+1][j][k]$ 的定义和 $\textit{dfs}(i,j,k)$ 的定义是一样的，都表示在 $[0,i]$ 中选数字，且在 $[i+1,n-1]$ 中选的两个不重叠子序列的 GCD 分别为 $j,k$ 时，最终可以得到的合法子序列对的个数。这里 $+1$ 是为了把 $\textit{dfs}(-1,j,k)$ 这个状态也翻译过来，这样我们可以把 $f[0][j][k]$ 作为初始值。

相应的递推式（状态转移方程）也和 $\textit{dfs}$ 一样：

$$
f[i+1][j][k] = f[i][j][k] + f[i][\gcd(j,x)][k] + f[i][j][\gcd(k,x)]
$$
