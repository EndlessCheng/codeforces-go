...
由于题目保证 $\textit{target}$ 是严格递增的，且 $\textit{target}$ 的最后一个数 $\le n$，所以按照下面的做法，一定可以得到 $\textit{target}$。

1. 设 $\textit{target}$ 的最后一个数为 $\textit{mx}$。初始化指针 $i=0$ 指向 $\textit{target}$ 的第一个数。
2. 枚举 $x = 1,2,\ldots,\textit{mx}$。
3. 先把 $x$ 入栈。
4. 如果 $\textit{target}[i] = x$，那么 $x$ 是我们要的数，把 $i$ 加一，指向 $\textit{target}$ 的下一个数。
5. 如果 $\textit{target}[i] \ne x$，那么 $x$ 不是我们要的数，把 $x$ 出栈。
