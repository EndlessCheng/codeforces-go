...
把 $\textit{nums}[i]$ 改成 $v$ 后：

- 如果 $v$ 不在哈希表中，创建一个新的有序集合。
- 否则，设 $l$ 和 $r$ 是（$v$ 插入有序集合之前的）最小下标和最大下标。
- 如果 $l=r$（有序集合大小为 $1$），插入 $i$ 后就有两个质数了，把区间 $[\min(l,i),\max(r,i)]$ 加一。
- 如果 $i<l$，把区间 $[i,l-1]$ 加一。
- 如果 $i>r$，把区间 $[r+1,i]$ 加一。
- 其余情况，不修改区间。相比写法一，这可以减少更新线段树的次数。

对于旧值的处理逻辑是类似的，可以封装成一个函数。
...
class Solution {
    ...
    private void update(LazySegmentTree t, TreeSet<Integer> s, int i, int delta) {
        int l = s.first();
        int r = s.last();
        if (l == r) {
            t.update(Math.min(l, i), Math.max(r, i), delta);
        } else if (i < l) {
            t.update(i, l - 1, delta);
        } else if (i > r) {
            t.update(r + 1, i, delta);
...
public:
    vector<int> maximumCount(vector<int>& nums, vector<vector<int>>& queries) {
        ...
        auto update = [&](set<int>& s, int i, int delta) {
