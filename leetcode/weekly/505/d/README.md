...
对于「没有 $m$ 约束」的问题，我们除了要计算所选元素之和的最大值，还需要计算**选了多少个子数组**，这样才能与 $m$ 比大小，通过二分调控 $k$ 的值。
...
可能有多种选子数组的方式，都得到了相同的 $f[n]$。例如在 $k=10$ 的情况下，选 $3$ 个、$4$ 个还是 $5$ 个子数组，都得到了相同的 $f[n]$。

在 DP 值相等时，我们的策略是优先选 $\textit{cnt}$ 值更小的。所以可能会出现 $m=4$，但最终二分出来的 $\textit{cnt}[n] = 3$ 的情况，怎么办？

既然选 $3$ 个子数组还是选 $4$ 个子数组都是一样的，那么直接认为 $f[n]$ 对应着我们选了 $m=4$ 个子数组。最终答案为 $f[n]$ 加上减去的 $m$ 个 $k$，即 $f[n] + mk$。
...
**问**：本题至少要选一个子数组，如果改成至少要选 $\textit{least}$ 个子数组（$\textit{least}$ 是额外传入的一个正整数），怎么做？

**答**：首先，跑一遍 $k=0$ 的算法，设我们选了 $c$ 个子数组。

分类讨论：

- 如果 $\textit{least}\le c \le m$，那么直接算出了答案。
- 如果 $c > m$，那么和本题一样，转化成恰好选 $m$ 个子数组的问题。
- 如果 $c < \textit{least}$，则转化成恰好选 $\textit{least}$ 个子数组的问题。
