枚举 $\textit{nums}$ 的子集作为 $B$，这样我们只需专注 XOR 的问题。
...
- 如果 $S$ 在这一位上一共有奇数个 $1$，那么无论怎么划分，一定是奇数个 $1$ 的异或和 + 偶数个 $1$ 的异或和 = $1$。即 $\text{XOR}(A) + \text{XOR}(C)$ 的这一位恒 $1$。
  - 由于奇数个 $1$ 的异或和是 $1$，偶数个 $1$ 的异或和是 $0$，所以 $S$ 中的包含奇数个 $1$ 的比特位所组成的二进制数，就是 $\text{XOR}(a)$。
- 如果 $S$ 在这一位上一共有偶数个 $1$，那么可以是：
  ...
  - 只考虑有偶数个 $1$ 的比特位（称其为特殊比特位），把 $a[i]$ 中的非特殊比特位置为 $0$，得到一个新的数组 $a'$。然后把 $a'$ 划分为 $A'$ 和 $C'$，那么 $\text{XOR}(A')=\text{XOR}(C')$，所以 $\text{XOR}(A') + \text{XOR}(C') = 2\cdot \text{XOR}(A')$。

所以 $\text{XOR}(A) + \text{XOR}(C)$ 的最大值就是

$$
\text{XOR}(a) + 2\cdot \text{XOR}(A')
$$

的最大值。

由于 $\text{XOR}(a)$ 是个定值，问题变成计算 $\text{XOR}(A')$ 的最大值，即
