...
如果 $x$ 不是质数，那么我们只能往 $i-1$ 和 $i+1$ 走一步。

如果 $x$ 是质数，我们除了能往 $i-1$ 和 $i+1$ 走一步，还能往 $x$ 在 $\textit{nums}$ 中的倍数那走一步。

所以要预处理每个质数的倍数的位置。

或者反过来，对于 $\textit{nums}$ 中的每个数 $x$，把 $x$ 的的质因子 $p$ 和 $x$ 的下标 $i$ 插入哈希表。哈希表的 key 是质数，value 是下标列表。

这样从哈希表中我们可以直接获取到 $x$ 在 $\textit{nums}$ 中的倍数的所有位置。

注意遍历下标列表后要把列表从哈希表中删除（或者清空），避免反复遍历列表。比如一个有很多质数 $2$ 的数组，我们把这些 $2$ 的下标入队，出队的时候，不能重复遍历 $2$ 的倍数的下标列表。
