...
对于字符不为 $\texttt{7}$ 或 $\texttt{9}$ 的情况，定义一个类似爬楼梯的 DP，即 $f[i]$ 表示长为 $i$ 的只有一种字符的字符串所对应的文字信息种类数，我们可以将末尾的 $1$ 个、$2$ 个或 $3$ 个字符变成一个字母，接下来解决的问题（子问题）为：长为 $i-1,i-2,i-3$ 的只有一种字符的字符串所对应的文字信息种类数，即
