...
func Test(t *testing.T) {
	...
	exampleIns := [][]string{{`1`}, {`2`}, {`3`}, {`7`}, {`5000`}}
	exampleOuts := [][]string{{`12`}, {`54`}, {`246`}, {`106494`}, {`30228214`}}
	// TODO: 测试参数的下界和上界！
	// custom test cases or WA cases.
	//exampleIns = append(exampleIns, []string{``})
	//exampleOuts = append(exampleOuts, []string{``})
	targetCaseNum := 0
	if err := testutil.RunLeetCodeFuncWithCase(t, numOfWays, exampleIns, exampleOuts, targetCaseNum); err != nil {
		t.Fatal(err)
	}
}
