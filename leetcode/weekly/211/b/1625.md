...
根据 [裴蜀定理](https://oi-wiki.org/math/number-theory/bezouts/)，$r-s_1$ 必须是 $g = \gcd(a,10)$ 的倍数，即
...
上式表明，$s_1$ 可以变成的数，必须与 $s_1$ 关于模 $g$ 同余，所以 $s_1$ 可以变成的最小值为
...
- 如果 $\gcd(b,n)$ 是奇数，轮转一次后，原来的偶数下标变成奇数下标，我们能对偶数下标执行累加操作。
...
var findLexSmallestString = function(s, a, b) {
    ...
    function modify(t, start) {
        ...
        // 从 ch 到 ch%g，需要增加 inc（循环中会 %10 保证结果在 [0,9] 中）
        let inc = ch % g - ch + 10;
        if (inc === 0) { // 优化：inc 为 0 时，无需执行下面的 for 循环
            return;
        }
...
impl Solution {
    pub fn find_lex_smallest_string(s: String, a: i32, b: i32) -> String {
        ...
        for i in (0..n).step_by(step) {
            let mut t = format!("{}{}", &s[i..], &s[..i]).into_bytes();
...
**注**：本题可以做到 $\mathcal{O}(n)$，参考 [最小表示法](https://oi-wiki.org/string/minimal-string/)。
