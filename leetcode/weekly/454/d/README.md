...
- 否则，那么答案在 $y$ 到 $\textit{lca}$ 的路径中，我们需要计算从 $y$ 往上跳**至多** $\textit{disXY} - \textit{half}$ 的节点，就是从 $x$ 出发，距离 $x$ 至少 $\textit{half}$ 的节点。

**至少** $\textit{half}$ 可以转化成往上跳**至多** $\textit{half}-1$ 到达的最远节点，从这个节点再往上跳一步，就是至少 $\textit{half}$ 的最近节点。

现在，两种情况统一为：

- 从 $x$ 出发，往上跳**至多** $d$，所能到达的最远节点。
