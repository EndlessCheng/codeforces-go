...
func Test(t *testing.T) {
	...
	exampleIns := [][]string{{`[0,1,2,3,4]`, `[0,1,2,2,1]`}, {`[1,2,3,4,0]`, `[0,1,2,3,0]`}, {`[1]`, `[0]`}}
	exampleOuts := [][]string{{`[0,4,1,3,2]`}, {`[0,1,2,3,4]`}, {`[1]`}}
	// TODO: 测试参数的下界和上界！
	// custom test cases or WA cases.
	//exampleIns = append(exampleIns, []string{``})
	//exampleOuts = append(exampleOuts, []string{``})
	targetCaseNum := 0
	if err := testutil.RunLeetCodeFuncWithCase(t, createTargetArray, exampleIns, exampleOuts, targetCaseNum); err != nil {
		t.Fatal(err)
	}
}
