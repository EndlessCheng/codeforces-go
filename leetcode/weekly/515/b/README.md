把所有汽车分配到持续时间为 $m = \max(\textit{lights})$ 的信号灯是最优的，红灯最短。

对于 $r\ge m$ 的汽车，计算 $\textit{period}-r$ 的最大值，即为答案。
...
- 时间复杂度：$\mathcal{O}(n+m)$，其中 $n$ 是 $\textit{lights}$ 的长度，$m$ 是 $\textit{arrivalTime}$ 的长度。
- 空间复杂度：$\mathcal{O}(1)$。
