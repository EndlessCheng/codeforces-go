...
- $A$ 和 $B$ 都可用：用总时长减去上面三种情况，得到 $A$ 和 $B$ 都可用的时长 $t_C = t - \left\lfloor\dfrac{t}{r_1}\right\rfloor - \left\lfloor\dfrac{t}{r_2}\right\rfloor + \left\lfloor\dfrac{t}{L}\right\rfloor$。注：也可以用容斥原理计算。
...
$d_1$ 消耗掉 $t_A$ 后，剩余值为 $d'_1 = \max(d_1-t_A, 0)$。$d_2$ 消耗掉 $t_B$ 后，剩余值为 $d'_2 = \max(d_2-t_B, 0)$。
...
每经过 $r$ 小时，就需要花 $1$ 小时充电（最后一段不需要充电）。

比如 $d=6,r=3$，那么分成三段，除了最后一段不需要充电，前两段都需要充电。
...
小时用来充电。其中 $d-1$ 可以兼容 $d$ 刚好是 $r-1$ 的倍数的情况，此时最后一段是不需要充电的。
