...
- 时间复杂度：$\mathcal{O}(mn)$，其中 $m$ 和 $n$ 分别是 $\textit{grid}$ 的行数和列数。虽然我们写了个三重循环，但由于答案最大是 $\min(m,n)$，所以最内层的 `ans++` 最多执行 $\min(m,n)$ 次，所以瓶颈其实在遍历 $\textit{mat}$ 上，三重循环的时间复杂度为 $\mathcal{O}(mn + \min(m,n)) = \mathcal{O}(mn)$。
- 空间复杂度：$\mathcal{O}(mn)$。
