...
注：也可以在 `i + ans >= m` 或者 `j + ans >= n` 时跳出循环。但测试了一下，并没有很大提升。

#### 复杂度分析

- 时间复杂度：$\mathcal{O}(mn)$，其中 $m$ 和 $n$ 分别是 $\textit{grid}$ 的行数和列数。虽然我们写了个三重循环，但最内层的 `ans++` 最多执行 $\mathcal{O}(\min(m,n))$ 次，所以瓶颈其实在遍历 $\textit{mat}$ 上，三重循环的时间复杂度为 $\mathcal{O}(mn + \min(m,n)) = \mathcal{O}(mn)$。
- 空间复杂度：$\mathcal{O}(mn)$。
