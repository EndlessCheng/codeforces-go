...
上面的转移方程，先去掉第一个维度，得到 $f[j] = f[j] \vee f[j-v]$，其中 $0\le j-v < v$

进一步地，把一维数组压缩成一个二进制数 $f$，其中二进制从低到高第 $j$ 位为 $1$ 表示 $f[j]=\texttt{true}$，为 $0$ 表示 $f[j]=\texttt{false}$。

转移方程相当于取 $f$ 的低 $v$ 位，再左移 $v$ 位，然后与 $f$ 取按位或，即编程语言中的 `f |= (f & ((1 << v) - 1)) << v`，其中：
