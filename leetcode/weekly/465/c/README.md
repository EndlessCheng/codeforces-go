...
- 定义 $F(S,i)$ 表示保留 $S$ 中的大于 $i$ 的数的所有子集。或者说小于等于 $i$ 的数可以选或不选。

注意 $F(S,i)$ 是一个集合，其中的元素也是集合。

再来看 $S = \{1,3,4\}$ 这个例子：

- 包含 $4$ 的子集就是 $F(S,3) = \{\{4\},\{1,4\},\{3,4\},\{1,3,4\}\}$。
- 不含 $4$ 的子集就是 $F(S\setminus \{4\},3) = \{\varnothing,\{1\},\{3\},\{1,3\}\}$。

那么 $F(S,3)\cup F(S\setminus \{4\},3)$ 就是 $S$ 的所有子集了。

### 状态定义与状态转移方程

定义 $f[i][S]$ 表示保留 $S$ 中的大于 $i$ 的数的所有子集的对应二进制的最大值。或者说小于等于 $i$ 的数可以选或不选。
