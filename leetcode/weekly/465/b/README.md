...
题目要求把 $n$ 拆分成 $k$ 个数的乘积。数据范围显示 $k\le 5$，这可以暴搜吗？

考虑有多少种拆分方案。顺序不同也算不同，把 $2\times 3$ 和 $3\times 2$ 当成不同的拆分方案。

比如 $n=2^{16}$，分成 $k=5$ 个数的乘积，就相当于把 $16$ 拆分成 $5$ 个非负整数之和。这等价于把 $16$ 个无区别的小球放入 $5$ 个有区别的盒子中的方案数。

根据 [图解](https://leetcode.cn/problems/count-ways-to-make-array-with-product/solutions/2713481/tu-jie-zhi-yin-zi-fen-jie-fang-qiu-wen-t-fboo/)，方案数为 $\dbinom {20} 4 = 4845$。挺小的。

对于 $n=90720 = 2^5 \times 3^4\times 5\times 7$ 这种有很多质因子的数，由于每种质数互相独立，$k=5$ 时，方案数为 $\dbinom 9 4 \dbinom 8 4 \dbinom 5 4  \dbinom 5 4 = 220500$。

注意这里计算的是方案数，即搜索树的**叶子**个数。整棵搜索树的节点个数需要累加 $k=1,2,3,4,5$ 时的方案数。比如 $n=90720$ 时，总的节点个数就是

$$
\sum_{k=1}^{5}\dbinom {5+k-1} {k-1}  \dbinom {4+k-1} {k-1} \dbinom {1+k-1} {k-1}  \dbinom {1+k-1} {k-1} = 254816
$$
