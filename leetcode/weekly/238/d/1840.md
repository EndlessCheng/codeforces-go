...
1. 首先，用 3796 题的两次扫描法，算出每个编号为 $\textit{id}_i$ 的建筑的最大高度。注意建筑 $\textit{id}_i$ 不仅受到 $\textit{maxHeight}_i$ 的约束，还因为「相邻建筑的高度差不能超过 $1$」会受到其他 $\textit{maxHeight}_j$ 的约束。
2. 然后，计算编号在 $\textit{id}_i$ 和 $\textit{id}_{i+1}$ 之间的建筑的最大高度。
...
编号在 $[\textit{id}_i,\textit{id}_{i+1}]$ 中的建筑的最大高度为

$$
\max_{j=\textit{id}_i}^{\textit{id}_{i+1}} \min(H[i] + j - \textit{id}_i, H[i+1] + \textit{id}_{i+1} - j)
$$
...
> 
> **注 2**：另一种计算方法是，设峰顶为 $j$，从 $\textit{id}_i$ 到 $j$ 再到 $\textit{id}_{i+1}$ 是先递增再递减的（如果只是递增或递减，那么峰顶在端点）。从 $\textit{id}_i$ 到 $j$，建筑高度至多增加了 $h_j - H[i]$；从 $\textit{id}_{i+1}$ 到 $j$，建筑高度至多增加了 $h_j - H[i+1]$。这两个增量之和不超过 $\textit{id}_{i+1} - \textit{id}_i$，所以有
> 
> $$
...
代码实现时，可以添加两个约束 $[1,0]$ 和 $[n,\infty]$，从而简化代码逻辑。见 Python3 代码。
...
int maxBuilding(int n, int** restrictions, int restrictionsSize, int* restrictionsColSize) {
    ...
    qsort(restrictions, m, sizeof(int*), cmp);
