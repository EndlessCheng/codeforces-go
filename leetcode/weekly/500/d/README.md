...
1. 由于固定点的值等于其下标，下标是严格递增的，所以**子序列的值必须是严格递增的**。
2. 对于两个固定点 $i$ 和 $j$（$i<j$），由于 $j$ 左侧删除的元素，包含 $i$ 左侧删除的所有元素，所以固定点左侧「删除的元素个数」是递增的，即 $i-\textit{nums}[i]$ **是递增的**（允许相邻相等）。
...
**问**：为什么可以排序，万一我们选出的子序列在原数组中的下标不是递增的呢？
