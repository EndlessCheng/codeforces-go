...
- $|x|=|y|$。必须满足 $\text{reverse}(x) = y$。这类似**最长公共子串**问题，见 [718. 最长重复子数组](https://leetcode.cn/problems/maximum-length-of-repeated-subarray/)。
- $|x|>|y|$。必须满足 $x$ 的长为 $|x|-|y|$ 后缀是一个回文串。设 $x$ 去掉该回文串后是 $x'$，还必须满足 $\text{reverse}(x') = y$。
- $|x|<|y|$。我们只需要计算 $|x|\ge |y|$ 的情况，对于 $|x| < |y|$ 的情况，等同于 $s=\text{reverse}(t)$，$t=\text{reverse}(s)$，可以复用同一个逻辑。

## |x| = |y|

类似 718 题，定义 $f[i+1][j]$ 表示以 $s[i]$ 结尾的子串（倒序）与以 $t[j]$ 开头的子串的**最长匹配长度**。这里 $+1$ 是为了用 $f[0][j]$ 表示初始值。如果不加一，就需要特判 $i=0$ 的情况。

分类讨论：

- 如果 $s[i]\ne t[j]$，那么 $f[i+1][j] = 0$。
- 如果 $s[i]=t[j]$，那么去掉这两个字母，问题变成以 $s[i-1]$ 结尾的子串（倒序）与以 $t[j+1]$ 开头的子串的最长匹配长度，即 $f[i+1][j] = f[i][j+1] + 1$。相当于在以 $s[i-1]$ 结尾的子串后面添加 $s[i]$，在以 $t[j+1]$ 开头的子串前面添加 $t[j]$。
