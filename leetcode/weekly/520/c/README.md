...
- 如果我们操作的是一个长为**奇数**的子数组。假设子数组为 $[a,b,c]$，操作后的顺序为 $[b,c,a]$。$a$ 的符号没变，其余元素在交替和中的正负号都取反了。所以该操作取反了偶数个元素。

综上，操作相当于：

- 选择 $\textit{nums}$ 中的一个长为**偶数**的子数组，把每个元素都取反。

交替和中的 $\textit{nums}[i]$，操作前是 $(-1)^i \textit{nums}[i]$，操作后是 $- (-1)^i \textit{nums}[i]$，增加了 $b[i] = -2(-1)^i \textit{nums}[i] = 2(-1)^{i+1} \textit{nums}[i]$。
