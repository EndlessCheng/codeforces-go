大的思路是，枚举 $j$，同时维护左边能构成影子对的 $\textit{nums}[i]$。

根据题意，一旦 $\textit{nums}[i]$ 右边出现比 $\textit{nums}[i]$ 小的数（作为 $\textit{nums}[k]$），那么 $\textit{nums}[i]$ 无法构成影子对。

比如示例 1 的 $[3,1,\ldots]$，由于 $1$ 的出现，$3$ 永远无法构成影子对。

用一个栈维护我们遍历过的数，如果当前元素 $x$ 比栈顶小，那么栈顶永远无法构成影子对，弹出栈顶。反复操作，直到栈为空，或者栈顶元素 $\le x$。

如此操作后，从栈底到栈顶是递增的。由于我们去掉了无法构成影子对的数，所以在 $x$ 左侧的能与 $x$ 构成影子对的数都在栈中。

但栈顶元素可能等于 $x$，不能计入。为此，我们可以改为在栈中保存 pair $(元素,元素的出现次数)$，同时维护栈的大小 $\textit{size}$。
