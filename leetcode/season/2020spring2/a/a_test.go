...
func Test(t *testing.T) {
	...
	exampleIns := [][]string{{`[1,2,3]`}, {`[1,1]`}, {`[1,1,2]`}}
	exampleOuts := [][]string{{`3`}, {`1`}, {`2`}}
	// TODO: 测试参数的下界和上界！
	// custom test cases or WA cases.
	//exampleIns = append(exampleIns, []string{``})
	//exampleOuts = append(exampleOuts, []string{``})
	targetCaseNum := 0
	if err := testutil.RunLeetCodeFuncWithCase(t, expectNumber, exampleIns, exampleOuts, targetCaseNum); err != nil {
		t.Fatal(err)
	}
}
