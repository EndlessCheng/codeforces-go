...
func Test(t *testing.T) {
	...
	exampleIns := [][]string{{`[1,2,3]`, `[1,2,3]`}, {`[2,2,3]`, `[3,2,1]`}}
	exampleOuts := [][]string{{`3`}, {`1`}}
	// TODO: 测试参数的下界和上界！
	// custom test cases or WA cases.
	//exampleIns = append(exampleIns, []string{``})
	//exampleOuts = append(exampleOuts, []string{``})
	targetCaseNum := 0
	if err := testutil.RunLeetCodeFuncWithCase(t, game, exampleIns, exampleOuts, targetCaseNum); err != nil {
		t.Fatal(err)
	}
}
