考察连续的回文数，例如 $232,242,252,262,\ldots$ 可以发现这些数的左半边（包含中心）是**连续**的 $23,24,25,26,\ldots$
...
> 注：代码实现时，无需考虑 $\textit{left}-1$ 十进制长度小于 $m$ 的情况，算出的回文数不如 $10^{m-1}-1$。同理也无需考虑 $\textit{left}+1$ 十进制长度大于 $m$ 的情况。
