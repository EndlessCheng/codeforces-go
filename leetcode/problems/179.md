...
能否推广这个思路？对于两个字符串 $a$ 和 $b$，如果 $a+b > b+a$，那么 $a$ 要排在 $b$ 的左边。

**猜想**：按照上述规则把 $\textit{nums}$ 排序后，拼接 $\textit{nums}$ 即为最优解。

## 我们需要证明什么？

1. 证明这个规则是没有矛盾的（有传递性），不会出现 $a$ 排在 $b$ 的左边，$b$ 排在 $c$ 的左边，$c$ 排在 $a$ 的左边。
