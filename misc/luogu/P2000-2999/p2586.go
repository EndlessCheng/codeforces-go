...
// 向量基本运算
func (a vec586) add(b vec586) vec586 { return vec586{a.x + b.x, a.y + b.y} }
func (a vec586) sub(b vec586) vec586 { return vec586{a.x - b.x, a.y - b.y} }
func (a vec586) len() float64        { return math.Hypot(float64(a.x), float64(a.y)) }
func (a vec586) len2() int           { return a.x*a.x + a.y*a.y }
func (a vec586) dot(b vec586) int    { return a.x*b.x + a.y*b.y }
func (a vec586) det(b vec586) int { return a.x*b.y - a.y*b.x }
func (a line586) vec() vec586     { return a.p2.sub(a.p1) }

// 点到线段的距离
func (a vec586) disToSeg(l line586) float64 {
...
func newGame(height, width int) *game {
	g := &game{height: height, width: width, cake: vec586{height, width}}
...
func (g *game) newAnt() {
	...
	if len(g.ants) >= maxAnts || g.board[0][0].hasObj {
		return
	}
	g.board[0][0].hasObj = true
...
// 格点 p 是否可达
func (g *game) canReach(p vec586) bool {
...
// 东南西北
var dir4 = [4]vec586{{0, 1}, {1, 0}, {0, -1}, {-1, 0}}
...
func (g *game) moveAnts() {
	...
	for _, a := range g.ants {
		...
		if maxPheromone == -1 {
			...
			// 更新蚂蚁位置和地图状态
			g.board[a.pos.x][a.pos.y].hasObj = false
			a.prevPos, a.pos = a.pos, a.pos.add(dir4[dirI])
			g.board[a.pos.x][a.pos.y].hasObj = true
		}
		// 如果蛋糕没被扛走并且有蚂蚁在蛋糕的位置上，它把蛋糕扛上，血量增加
		if g.antWithCake == nil && a.pos == g.cake {
			a.getCake()
			g.antWithCake = a
		}
	}
...
func (g *game) towerAttack() {
	...
	for _, t := range g.towers {
		...
		if g.antWithCake != nil && t.pos.sub(g.antWithCake.pos).len2() <= t.atkRange*t.atkRange {
			...
			towerToAntSeg := line586{t.pos, g.antWithCake.pos}
			for _, a := range g.ants {
				if a.pos.disToSeg(towerToAntSeg)-eps586 < antRadius {
					a.curHP -= t.damage
				}
			}
		} else {
			// 否则塔会攻击离它最近的蚂蚁，如果有多只蚂蚁，它会选出生较早的一只
...
func p2586(_r io.Reader, _w io.Writer) {
	...
	for i := range g.towers {
		var x, y int
		Fscan(in, &x, &y)
		g.towers[i] = &tower{damage, atkRange, vec586{x, y}}
