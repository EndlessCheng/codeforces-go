...
type seg1712 []struct{ l, r, mx, todo int }

func (t seg1712) apply(o, f int) {
	cur := &t[o]
	cur.mx += f
	cur.todo += f
}

func (t seg1712) maintain(o int) {
	t[o].mx = max(t[o<<1].mx, t[o<<1|1].mx)
}

func (t seg1712) spread(o int) {
	f := t[o].todo
	if f == 0 {
		return
	}
...
func (t seg1712) build(o, l, r int) {
	t[o].l, t[o].r = l, r
	if l == r {
		return
	}
...
func p1712(in io.Reader, out io.Writer) {
	...
	t := make(seg1712, 2<<bits.Len(uint(k-1)))
	t.build(1, 0, k-1)
	ans := int(2e9)
	left := 0
	for _, p := range a {
