...
func NewOrderedSetWithSpace(space int) OrderedSet {
	return OrderedSet{make(map[any]*list.Element, space), list.New()}
}
...
// 判断 v 是否存在
func (s *OrderedSet) Contains(v any) bool {
	_, ok := s.set[v]
	return ok
}
...
// 按照插入顺序遍历哈希表中的元素
// 当 f 返回 false 时停止遍历
func (s *OrderedSet) Range(f func(key any) bool) {
