...
a+b = (a|b) + (a&b)
    = (a&b)*2 + (a^b)
    = (a|b)*2 - (a^b)
a^b = a+b - (a&b)*2  注：这个结论可以用于字符串匹配，见 math_fft.go
(a^b) & (a&b) = 0 恒成立
...
func _(x int, abs func(int) int) {
	...
	fastNumLength := func() {
		...
		for i := range lenPow2 {
			...
			// 现在有 2^i < pow10
			lenPow2[i] = len10          // 2^i 的十进制长度
			pow2ToPow10[i] = pow10 / 10 // 10^(len10-1)
		}

		// 返回 x 的十进制长度 len10，以及 base=10^(len10-1)
		// 时间复杂度 O(1)
		query := func(x int) (int, int) {
			len2 := bits.Len(uint(x)) // 如果是 int32 的数据范围，可以用 bits.Len32
			len10 := lenPow2[len2]
			base := pow2ToPow10[len2]
			if x > 0 && x < base {
				len10--
				base /= 10
			}
		...
		//for i := 0; i <= 1e7; i++ {
		//	ten := pow2ToPow10[bits.Len(uint(i))]
		//	l := lenPow2[bits.Len(uint(i))]
		//	if i > 0 && i < ten {
		//		ten /= 10
		//		l--
		//	}
		//	if len(strconv.Itoa(i)) != len(strconv.Itoa(ten)) {
		//		panic(-1)
		//	}
		//	if len(strconv.Itoa(i)) != l {
		//		panic(-1)
		//	}
		//}
